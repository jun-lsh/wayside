/*
 * bt_task.c - BLE Advertising and Scanning Task
 *
...
/**
 * @brief BT controller callback - controller ready to receive command
 */
static void controller_rcv_pkt_ready(void)
{
...
/**
 * @brief BT controller callback - transfer data packet to host
 */
static int host_rcv_pkt(uint8_t *data, uint16_t len)
{
    ...
    /* Check for HCI Command Complete event (0x0e) */
    if (data[1] == 0x0e) {
        if (data[6] == 0) {
...
static void hci_cmd_send_ble_scan_params(void)
{
    ...
    /* Scan window and interval in slots (625 μs each) */
    uint16_t scan_interval = 0x50; /* 50 ms */
    uint16_t scan_window = 0x30;   /* 30 ms */

    uint8_t own_addr_type = 0x00;  /* Public Device Address */
    uint8_t filter_policy = 0x00; /* Accept all packets */
    
    uint16_t sz = make_cmd_ble_set_scan_params(hci_cmd_buf, scan_type, 
...
static void hci_cmd_send_ble_set_adv_param(void)
{
    /* Advertising interval in slots (625 μs each) */
    ...
    uint8_t adv_chn_map = 0x07;    /* Channels 37, 38, 39 */
    uint8_t adv_filter_policy = 0; /* Accept all */
...
static esp_err_t get_local_name(uint8_t *data_msg, uint8_t data_len, ble_scan_local_name_t *scanned_packet)
{
    ...
    while (curr_ptr < data_len) {
        ...
        /* Search for name data types (0x08 or 0x09) */
        if (curr_type == 0x08 || curr_type == 0x09) {
            for (uint8_t i = 0; i < curr_len - 1; i++) {
...
esp_err_t bt_init(void)
{
    ...
    if (ret) {
        ESP_LOGI(TAG, "Bluetooth controller release classic bt memory failed: %s", 
                 esp_err_to_name(ret));
        return ret;
    }
    ...
    vhci_host_cb.notify_host_send_available = controller_rcv_pkt_ready;
    ...
    while (continue_commands) {
        if (esp_vhci_host_check_send_available()) {
            switch (cmd_cnt) {
                ...
                default: continue_commands = false; break;
            }
...
void hci_evt_process(void *pvParameters)
{
    ...
    while (1) {
        ...
        uint8_t *addr = NULL, *data_len = NULL, *data_msg = NULL;
        short int *rssi = NULL;
        uint16_t data_ptr;
        ...
        if (xQueueReceive(adv_queue, rcv_data, portMAX_DELAY) != pdPASS) {
            ...
            if (hci_event_opcode == LE_META_EVENTS) {
                ...
                if (sub_event == HCI_LE_ADV_REPORT) {
                    ...
                    /* Get address type for each report */
                    ...
                    /* Get data length for each report */
                    ...
                    for (uint8_t i = 0; i < num_responses; i++) {
                        ...
                        /* Print data if valid name found */
                        if (ret == ESP_OK) {
                            printf("******** Response %d/%d ********\n", i + 1, num_responses);
                            printf("Event type: %02x\nAddress type: %02x\nAddress: ", 
                                   event_type[i], addr_type[i]);
                            for (int j = 5; j >= 0; j--) {
...
reset:
            ...
            printf("Raw Data:");
            for (uint8_t j = 0; j < rcv_data->q_data_len; j++) {
