...
export default function DashboardStep({ 
  ...
  // 3. Decrypt Trigger - Only decrypt after we've uploaded our own selfie
  // This prevents a race condition where receiving the partner's image before
  // uploading our own would skip the upload flow entirely
  useEffect(() => {
    if (receivedEncryptedPayload && !partnerSelfie && !decrypting && selfieUploadComplete) {
