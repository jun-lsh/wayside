...
esp_err_t nfc_pair_write_ndef(void)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "failed to write ndef: %s", esp_err_to_name(ret));
        return ret;
    }
    
    // configure fd pin to signal when ndef is read
    ret = nfc_set_fd_mode(s_config.nfc, NFC_FD_OFF_LAST_NDEF, NFC_FD_ON_RF_ON);
    if (ret != ESP_OK) {
        ESP_LOGW(TAG, "failed to set fd mode: %s", esp_err_to_name(ret));
    }
